// _lib.js - shared in-memory store for all API routes
var fs = require("fs");
var _data = { students: [], courses: [], checkins: [], pauses: [] };

function loadData() {
  if (_data.students && _data.students.length === 0) {
    try { if (fs.existsSync("/tmp/data.json")) { var d = JSON.parse(fs.readFileSync("/tmp/data.json","utf-8")); if (d && d.students) _data = d; } } catch(e) {}
  }
  return _data;
}
...
function colAPI(req, res, colName) {
  var method = req.method.toUpperCase();
  if (method === "OPTIONS") { res.writeHead(204, CORS); res.end(); return; }
  var db = loadData(), coll = db[colName], url = new URL(req.url, "http://l");
  ...
  if (method === "GET" && !id) {
    ...
    if (sp.has("studentPhone") && colName==="courses") { var spp=sp.get("studentPhone"); f=coll.filter(function(x){return x.studentPhone===spp||x.studentId===spp}); }
