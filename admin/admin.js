...
function SW(n){
  ...
  document.getElementById("pt").textContent=["📊 仪表盘","👥 学员管理","📋 课程管理","✅ 签到记录","⏸️ 请假管理"][n];
...
function R0(){
  ...
  h+='<div class="stat-card"><div class="stat-icon s2">💰</div><div><div class="stat-val">¥'+rev+'</div><div class="stat-lbl">总营收</div></div></div>';
  ...
  h+='<div class="stat-card"><div class="stat-icon s5">⏸️</div><div><div class="stat-val">'+ps+'</div><div class="stat-lbl">暂停中</div></div></div></div>';
  h+='<div class="card"><div class="card-header"><h2>👥 学员概览</h2><div class="sub">'+D.s.length+' 人</div></div>';
  if(D.s.length===0)h+='<div class="empty"><div class="icon">👤</div><p>暂无学员</p></div>';
  else{h+='<table><tr><th>姓名</th><th>手机号</th><th>课程数</th></tr>';D.s.forEach(function(s){var n=D.c.filter(function(c){return c.studentId===s.id}).length;h+='<tr><td><b>'+E(s.name)+'</b></td><td>'+E(s.phone)+'</td><td>'+n+'</td></tr>'});h+='</table>'}h+='</div>';
  var rc=D.k.slice().sort(function(a,b){return(b.createTime||b.date||"").localeCompare(a.createTime||a.date||"")}).slice(0,8);
  h+='<div class="card"><div class="card-header"><h2>🕐 最近签到</h2></div>';
...
function R2(){
  ...
  else{h+='<table><tr><th>学员</th><th>类型</th><th>状态</th><th>价格</th><th>剩余</th><th>操作</th></tr>';D.c.forEach(function(c){var s=G(c.studentId);var isM=c.type==="monthly";var stT=c.status==="active"?"tag-a":c.status==="paused"?"tag-p":"tag-e";var stL=c.status==="active"?"进行中":c.status==="paused"?"暂停中":"已到期";h+='<tr><td><b>'+E(s.name)+'</b></td><td><span class="tag '+(isM?"tag-m":"tag-s")+'">'+(isM?"月卡":"次卡")+'</span></td><td><span class="tag '+stT+'">'+stL+'</span></td><td>¥'+(c.price||0)+'</td><td>'+(isM?(c.totalDays||30)+"天":(c.rem!==undefined?c.rem:c.total||10)+"/"+(c.total||10)+"次")+'</td><td><button class="btn btn-o btn-sm" data-ec="'+c.id+'">编辑</button> <button class="btn btn-d btn-sm" data-dc="'+c.id+'">删除</button></td></tr>'});h+='</table>'}h+='</div>';
  m.innerHTML=h;
}

function R3(){
  var m=document.getElementById("mc");
  var h='<div class="card"><div class="card-header"><h2>✅ 签到记录</h2><div class="sub">共 '+D.k.length+' 条</div></div>';
...
function R4(){
  ...
  var h='<div class="card"><div class="card-header"><h2>⏸️ 进行中的请假</h2><div class="sub">'+act.length+' 条</div></div>';
...
function editCourse(id){
  ...
  OM("编辑课程",'<div class="fg"><label>学员</label><select id="cs">'+opts+'</select></div><div class="fr"><div class="fg"><label>状态</label><select id="cst"><option value="active"'+(c.status==="active"?" selected":"")+'>进行中</option><option value="paused"'+(c.status==="paused"?" selected":"")+'>暂停中</option><option value="expired"'+(c.status==="expired"?" selected":"")+'>已到期</option></select></div><div class="fg"><label>价格 ¥</label><input id="cp" type="number" value="'+(c.price||0)+'"></div></div>',async function(){var r=await fetch(API+"/courses/"+id,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({studentId:document.getElementById("cs").value,status:document.getElementById("cst").value,price:Number(document.getElementById("cp").value)||0})});if(r.ok){await LD();SW(T);TX("更新成功")}else{TX("更新失败")}});
}
async function delCourse(id){if(!confirm("确定删除？"))return;var r=await fetch(API+"/courses/"+id,{method:"DELETE"});if(r.ok){await LD();SW(T);TX("已删除")}else{TX("删除失败")}}
...
async function resumePause(id){
  ...
  if(r.ok){if(p.courseId||p.cid){var cid=p.courseId||p.cid;await fetch(API+"/courses/"+cid,{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify({status:"active"})})}await LD();SW(T);TX("已恢复")}else{TX("恢复失败")}
}
...
document.addEventListener("click",function(ev){
  ...
  if(el.id==="br"||el.closest("#br")){LD().then(function(){SW(T);TX("已刷新")});return}
